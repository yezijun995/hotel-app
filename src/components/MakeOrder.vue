      ...
      <mu-card raised>
        ...
        <mu-card-text>
          <mu-form :model="form" class="mu-demo-form">
            <mu-form-item prop="username" :rules="nameRules" icon="person" label="入住人" label-float>
              <mu-text-field prop="username" :rules="nameRules" v-model="form.username"></mu-text-field>
            </mu-form-item>
    ...
    methods: {
      fetchData() {
        ...
        getRoomTypeById(this.roomTypeId).then(res => {
          ...
          this.form.roomTypeName = this.roomType.typeName
        })
      },
      goBack() {
        this.$router.back()
