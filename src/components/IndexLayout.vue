<template>
  <div id="index">
    <mu-appbar style="width: 100%;" class="appBar" z-depth="0" color="primary">
      ...
      <mu-menu slot="left">
        <mu-button flat><h3>{{hotelInfo.address}}</h3></mu-button>
        <mu-list slot="content">
          <mu-list-item button v-for="(item,i) in hotelInfos" :key="item.hotelId" @click="getWinshopInfo(item.hotelId)">
            <mu-list-item-content>
              <mu-list-item-title>{{item.address}}</mu-list-item-title>
            </mu-list-item-content>
          </mu-list-item>
        </mu-list>
      </mu-menu>
    ...
    <router-view :hotelInfo="hotelInfo" class="layout"/>
...
<script>
  ...
  import {list, getInfo} from "@/api/hotel"
  ...
  export default {
    ...
    created: function () {
      list().then(res => {
        this.hotelInfos = res.data
        this.hotelInfo = res.data[0]
        Cookies.set("hotelId",this.hotelInfo.hotelId)
        Cookies.set("address",this.hotelInfo.address)
      }).catch(err => {
        console.log(err)
    ...
    methods: {
      ...
      getWinshopInfo(hotelId) {
        getInfo(hotelId).then(res => {
