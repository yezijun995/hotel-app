  ...
  <div class="login-container">
    <mu-card class="login-card" raised>
      <mu-form ref="form" :model="validateForm" label-position="left" label-width="100" class="mu-demo-form">
        <mu-card-title title="Yifelix网上订购酒店" sub-title="用户注册"></mu-card-title>
        <mu-card-text>
          <mu-form-item label="用户名" help-text="" prop="accountName" :rules="rules.notNull">
            <mu-text-field v-model="validateForm.accountName"></mu-text-field>
          </mu-form-item>
          ...
          <mu-form-item label="昵称" help-text="" prop="nickname" :rules="rules.notNull">
            <mu-text-field v-model="validateForm.nickname"></mu-text-field>
          </mu-form-item>
          <mu-form-item label="姓名" help-text="" prop="realName" :rules="rules.notNull">
            <mu-text-field v-model="validateForm.realName"></mu-text-field>
          </mu-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      login() {
        accountRegister(this.validateForm).then(res => {
          console.log(res)
          if (res.code == 500) {
            this.$toast.message("注册失败：" + res.msg)
          }else{
