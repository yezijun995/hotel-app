          ...
          <mu-col xl="12" lg="12" md="12" sm="12" span="12">
            <mu-list-item avatar button :ripple="true" class="orderItem" v-for="(order,index) in orderList"
              ...
              <mu-list-item-content>
                ...
                <mu-list-item-sub-title>预订日期：{{ order.orderDate | formatDate }} 到 {{ order.orderDate |
                  addDate(order.orderDays) }}
                </mu-list-item-sub-title>
                <mu-list-item-sub-title>订单消费：￥ {{ order.orderCost }}</mu-list-item-sub-title>
              </mu-list-item-content>
              <mu-list-item-action>
                <mu-badge :content="order.orderState|getOrderStatus"
                          :color="order.orderState|getStatusColor"></mu-badge>
