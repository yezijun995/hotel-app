<template>
  <div id="orderDetail">
      ...
      <mu-row>
        <mu-col>
          <mu-card class="card" raised>
            <mu-card-header :title="'订单号: '+  order.ordersId ">
            ...
            <mu-card-text>
              <mu-list>
                <mu-list-item>
                  ...
                  <mu-list-item-title>房间号： {{ order.number }}</mu-list-item-title>
                </mu-list-item>
                <mu-list-item v-show="order.orderState === 2">
                  <mu-list-item-action>
                    <mu-icon value="hotel"></mu-icon>
                  </mu-list-item-action>
                ...
                <mu-list-item>
                  ...
                  <mu-list-item-title>预订日期：{{ order.orderDate | formatDate }} 到 {{
                    order.orderDate|addDate(order.orderDays) }}
                  </mu-list-item-title>
                </mu-list-item>
                <mu-list-item>
                  <mu-list-item-action>
                ...
                <mu-list-item>
                  ...
                  <mu-list-item-title>入住人： {{ order.username }}</mu-list-item-title>
                </mu-list-item>
                <mu-list-item>
                  <mu-list-item-action>
                    <mu-icon value="phone"></mu-icon>
            ...
            <mu-card-actions class="actions">
              ...
              <mu-button style="width: 49%" flat :color="order.orderState|getStatusColor">
    ...
    <mu-dialog title="正在付款" width="360" :open.sync="openPass" :loading="payLoading">
      ...
      <mu-text-field label="Password" v-model="password" label-float error-text="请输入支付密码" type="password"
  ...
  export default {
    ...
    methods: {
      ...
      cancelOrder() {
        ...
        cancelOrder(this.order.ordersId).then(res => {
          if (res.code === 200) {
